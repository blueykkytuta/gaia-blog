...
function moveTooltip(e) {
  ...
  // 默认：标签底部中心放到鼠标右上方较远处（translate(-50%,-100%) 使本体向上展开，远离圆圈）
  let x = (e.clientX - rect.left) / scale + 40;
  let y = (e.clientY - rect.top) / scale - 90;
  if (x > 680) x = (e.clientX - rect.left) / scale - 280;   // 右侧不够 → 翻到左侧远处
  if (y < 120) y = (e.clientY - rect.top) / scale + 130;    // 顶部不够 → 翻到鼠标下方远处

...
function moveTooltip(e) {
  ...
  // 与交互图完全一致：直接用 CSS 像素坐标（不换算 viewBox scale）
  // .tooltip 的 transform translate(-50%,-100%) 使标签底部中心锚定在鼠标位置上方，本体向上展开
  // 任何窗口宽度下标签都稳定在鼠标正上方，不会因 scale 漂移遮挡圆圈
...
// ---------- 行业多选 UI ----------
// ---------- 行业多选（按 5 大类色分行显示，顺序同相对强度页） ----------
// 采用相对强度页的行业顺序（银行 → 综合），与"相对强度表格"页保持一致
const FILTER_ORDER = [
  "银行","石油石化","有色金属","煤炭","钢铁","公用事业",
  "非银金融","电子","通信","计算机","传媒","电力设备","汽车","机械设备",
  "食品饮料","家用电器","轻工制造","美容护理","纺织服饰","商贸零售","社会服务",
  "房地产","建筑材料","建筑装饰",
  "医药生物","基础化工","交通运输","国防军工","农林牧渔","环保","综合"
];
// 分组边界（按 FILTER_ORDER 从左到右），颜色与相对强度页一致
const FILTER_BANDS = [
...
const _bandEndIdx = FILTER_BANDS.map(b => FILTER_ORDER.indexOf(b.end));

/* ============================================================
   Carrycore 文章图片灯箱（v2 · 2026-09-21）
   ------------------------------------------------------------
   ...
   v2 相对 v1 的唯一改动：unlockScroll() 关闭时不再「滚一下」（详见该函数注释）
       v1 = js/lightbox-0921.js（保留，供缓存中的旧 HTML 引用）
...
(function () {
  ...
  function unlockScroll() {
    /* ✅ 2026-09-21 修复：关闭灯箱时页面会「自己上下滚一下」
       ------------------------------------------------------------
       根因：站点 css/style.css 有一条全局规则 html{scroll-behavior:smooth}
             （给导航锚点用的，不能删）。锁定期间 body 被 fixed，文档滚动位置
             被压成 0；下面这句 window.scrollTo(0, lockY) 也被 smooth 接管，
             于是「恢复位置」变成一段约 600ms 的平滑动画，肉眼就是滚了一下。
       实测 A/B（唯一变量就是本函数这三行）：
             原状   scrollY 取 19 个中间值  0→2→12→…→363，耗时 606ms
             修复后 scrollY 取 2 个值      0→363，一帧到位
       ------------------------------------------------------------
       只把「这一次滚动」临时改成 auto，用完立刻还原。
       ⚠️ 还原必须写 ''（清除内联）而不是硬写 'smooth'：
          否则 css 里针对 prefers-reduced-motion 的降级会被内联 style 压过，
          开了「减少动效」的用户反而被强行喂动画。
       ⚠️ scroll-behavior 不继承：实测 html=smooth / body=auto / .cc-lb-stage=auto，
          所以这里只影响文档这一次滚动，动不了任何子元素和其他页面。 */
    var b = doc.body;
    ...
    try {
      ...
      root.style.scrollBehavior = '';   // 立刻还原，页面其他滚动照旧是平滑的
    }
  }
